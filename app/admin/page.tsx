...
export default function AdminPage() {
  ...
  const filteredBookings = useMemo(() => {
    if (!bookingSearchTerm.trim()) return bookings
    ...
    return bookings.filter((booking) => {
      return (
        ...
        new Date(booking.date).toLocaleDateString().toLowerCase().includes(searchLower) ||
        booking.time.toLowerCase().includes(searchLower) ||
        (booking.occupation && booking.occupation.toLowerCase().includes(searchLower)) ||
        (booking.message && booking.message.toLowerCase().includes(searchLower))
      )
    })
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((user) => {
      return (
        ...
        (user.isEmailVerified ? "verified" : "unverified").includes(searchLower)
      )
    })
  ...
  if (loading || isLoading) {
    return (
      <div className="min-h-screen flex flex-col luxury-gradient">
        <Header />
        <main className="flex-1 flex mt-8 items-center justify-center">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-amber-600 mx-auto mb-4"></div>
            <p className="text-gray-600">Loading admin dashboard...</p>
          </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="bookings">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 space-y-4 sm:space-y-0">
                  <h2 className="text-2xl sm:text-3xl font-playfair font-bold text-gray-900 text-center sm:text-left">
                    All Appointments
                  </h2>
                  ...
                  <div className="relative w-full sm:w-80">
                    ...
                    <Input
                      type="text"
                      placeholder="Search bookings by name, email, service, status..."
                ...
                {bookingSearchTerm && (
                  <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                    <p className="text-sm text-amber-800">
                      ...
                      Found {filteredBookings.length} booking{filteredBookings.length !== 1 ? "s" : ""} matching "
                      {bookingSearchTerm}"
                      {filteredBookings.length !== bookings.length && (
                        <span className="ml-2 text-amber-600">(out of {bookings.length} total)</span>
                      )}
                    </p>
                  </div>
                )}
                      ...
                      {bookingSearchTerm ? (
                        <>
                          ...
                          <p className="text-gray-600 text-sm sm:text-base mb-4">
                            No bookings match your search criteria "{bookingSearchTerm}".
                          </p>
                          <Button onClick={clearBookingSearch} variant="outline" className="bg-white">
                            Clear Search
                          </Button>
                        </>
                      ) : (
              ...
              <TabsContent value="users">
                ...
                {userSearchTerm && (
                  <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                    <p className="text-sm text-amber-800">
                      <Search className="w-4 h-4 inline mr-2" />
                      Found {filteredUsers.length} user{filteredUsers.length !== 1 ? "s" : ""} matching "
                      {userSearchTerm}"
                      {filteredUsers.length !== users.length && (
                        <span className="ml-2 text-amber-600">(out of {users.length} total)</span>
                      )}
                    </p>
                  </div>
                )}
                    ...
                    <CardContent className="p-8 sm:p-12 text-center">
                      {userSearchTerm ? (
                        <>
                          ...
                          <Button onClick={clearUserSearch} variant="outline" className="bg-white">
                            Clear Search
                          </Button>
                        </>
                      ) : (
