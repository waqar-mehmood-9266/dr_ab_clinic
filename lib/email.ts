...
export async function sendEmailVerification(user: any, token: string) {
  const verificationUrl = `${process.env.NEXT_PUBLIC_BASE_URL || "https://www.drabaestheticandlaserskinclinic.com.au/"}auth/verify-email?token=${token}`
...
export async function sendPasswordResetEmail(user: any, token: string) {
  const resetUrl = `${process.env.NEXT_PUBLIC_BASE_URL || "https://www.drabaestheticandlaserskinclinic.com.au/"}auth/reset-password?token=${token}`
